...
### Demographic
Among the 32 columns with less than 25% of missing value we have selected eight relevant columns:
...
* **Citizenship_status (DMDCITZN)** - {Are you/Is SP} a citizen of the United States?
...
Among the 88 columns with less than 25% of missing value we have selected sixty-nine relevant columns:
...
* **Food_yesterday (DR1_300)** - Was the amount of food that {you/NAME} ate yesterd… more than usual, usual, or much less than usual?.
* **Water_yesterday (DR1_320Z)** - Total plain water drank yesterday - including plai… a water cooler, bottled water, and spring water..
...
Among the 105 columns with less than 25% of missing value we have selected <which> relevant columns:

* **ID (SEQN)** - Respondent sequence number.

examination_MS <- examination %>% summarise_all(~(sum(is.na(.))/n()))
examination_MS <- gather(examination_MS, key = "variables", value = "percent_missing")
examination_MS[examination_MS[2]<0.25,][[1]]


### Labs
<!-- https://wwwn.cdc.gov/nchs/nhanes/search/variablelist.aspx?Component=Laboratory&CycleBeginYear=2013 -->
Among the 46 columns with less than 25% of missing value we have selected six relevant columns:
...
* **Supplements (PHQ060)** - Dietary supplements such as vitamins and minerals?…e multivitamins and single nutrient supplements.].
* **Cholesterol (LBXTC)** - Total Cholesterol( mg/dL).


### Medication
<!-- https://wwwn.cdc.gov/Nchs/Nhanes/2013-2014/RXQ_RX_H.htm -->
All of the columns had more than 25% missing value. Among the 8 columns with less than 32% of missing value we have selected five relevant columns:
...
Among the 79 columns with less than 25% of missing value we have selected <which> relevant columns:

* **ID (SEQN)** - Respondent sequence number.
questionnaire_MS <- questionnaire %>% summarise_all(~(sum(is.na(.))/n()))
questionnaire_MS <- gather(questionnaire_MS, key = "variables", value = "percent_missing")
questionnaire_MS[questionnaire_MS[2]<0.25,][[1]]
